    ...
    public static void maximumValue(int knapsackSize, int[] weight, int[] value) {
        ...
        for (int i = 1; i <= nItems; i++) { // outer loop iterating through weights
            ...
            for (int j = 1; j <= knapsackSize; j++) {
                ...
                if (weight[i - 1] <= j) { // Can we pick the item
        ...
        while (i > 0 && j > 0) {
            if (matrix[i][j] == matrix[i - 1][j]) { // You didn't pick this item
                i--;	// Consider first i-1 items
            } else {
                System.out.println("Selected item no. " + i + " of Weight: " + weight[i - 1]);
                //       selected[i - 1] = 1;
                j = j - weight[i - 1]; //Set up j because you picked the item
                			// j=the remaining weight after you've picked the item
                			//  = j-weight[i-1]
                i--; // You've picked the item...go for i-1 item for max weight= j-weight[i-1]
            }
        }
